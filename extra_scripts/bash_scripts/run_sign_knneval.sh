...
do
    ...
    checkpoint_dir="./outputs/knneval/sign_grid/sign_with_param_${k}"

    use_sign_layer=True
    test_only=False
    num_gpus=7

    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,7 python tools/nearest_neighbor_test.py config=${main_config} config.DISTRIBUTED.NUM_PROC_PER_NODE=${num_gpus} config.TEST_ONLY=${test_only} config.MODEL.WEIGHTS_INIT.PARAMS_FILE=${params_file} config.CHECKPOINT.DIR=${checkpoint_dir} config.MODEL.TRUNK.RESNETS.USE_SIGN_LAYER=${use_sign_layer} config.MODEL.TRUNK.RESNETS.SIGN_PARAM_VALUE=${k}
    echo "Done with $k!"
